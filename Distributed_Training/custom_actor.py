#copied from https://github.com/tensorflow/agents/blob/v0.15.0/tf_agents/train/actor.py
...
class CustomActor(object):
  ...
  def __init__(self,
    ...
    if isinstance(env, py_environment.PyEnvironment):
      self._driver = custom_py_driver.CustomPyDriver(
          env,
          policy,
          self._observers,
  ...
  def run(self):
    next_policy_action, self._policy_state = self._driver.get_next_policy_state(
  ...
  def get_run_results(self):
      self._time_step, self._policy_state = self._driver.run(self._time_step, self._policy_state)
