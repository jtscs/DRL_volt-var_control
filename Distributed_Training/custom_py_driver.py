#copied from https://github.com/tensorflow/agents/blob/a992617f86cbfb02a6e3cf5a4f3de1c493a13318/tf_agents/drivers/py_driver.py
...
class CustomPyDriver(driver.Driver):
  ...
  def __init__(
    ...
    **Note** about bias when using batched environments with `max_episodes`:
    ...
    self.max_episodes = max_episodes or 0
    if self.max_steps < 1 and self.max_episodes < 1:
      raise ValueError(
          'Either `max_steps` or `max_episodes` should be greater than 0.')

    super(CustomPyDriver, self).__init__(env, policy, observers, transition_observers)
    self._max_steps = self.max_steps or np.inf
    self._max_episodes = self.max_episodes or np.inf
    self._end_episode_on_boundary = end_episode_on_boundary

  def get_next_policy_state(
      self,
    ...
    if not self.env.batched and time_step.is_first() and self.num_episodes > 0:
      policy_state = self._policy.get_initial_state(self.env.batch_size or 1)

    self.action_step = self.policy.action(time_step, policy_state)

    return self.action_step.action, policy_state
  ...
  def run(
    ...
    next_time_step = self.env.step(self.action_step.action)
    ...
    action_step_with_previous_state = self.action_step._replace(state=policy_state)
    ...
    self.num_steps += np.sum(~traj.is_boundary())

    time_step = next_time_step
    policy_state = self.action_step.state
